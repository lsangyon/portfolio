...
이 컴파일러는 Pascal 계열 프로그램을 직접 실행하는 것이 아니라, Lexer를 통해 생성된 `.ts` 토큰 파일을 입력으로 받아 CASL II 코드로 변환합니다. 아래 예시는 간단한 Pascal 계열 프로그램이 컴파일 과정을 거쳐 CASL II 코드로 변환되는 흐름을 보여주기 위한 예시입니다.
...
위와 같은 Pascal 계열 프로그램은 Lexer를 통해 `.ts` 토큰 파일로 변환됩니다. Compiler 단계에서는 이 `.ts` 파일을 입력으로 받아 문법 분석, 의미 분석, 코드 생성을 수행합니다.
...
생성되는 CASL II 코드는 테스트 파일과 구현 방식에 따라 달라질 수 있습니다. 아래 코드는 변수 대입, 산술 연산, 출력 처리가 CASL II 명령 흐름으로 변환되는 방식을 보여주기 위한 축약 예시입니다.
...
이 예시를 통해 입력 프로그램의 변수 대입, 산술 연산, 출력 처리가 CASL II 코드의 스택 연산, 레지스터 연산, 라이브러리 호출 형태로 변환되는 흐름을 확인할 수 있습니다.
